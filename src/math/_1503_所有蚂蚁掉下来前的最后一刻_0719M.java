 ...
 */
public class _1503_所有蚂蚁掉下来前的最后一刻_0719M {
    /*
     两只相遇的蚂蚁同时改变移动方向之后的情形
     等价于两只蚂蚁都不改变移动方向
     这样问题就简化成根据每只蚂蚁的初始位置和移动方向得到最后一只蚂蚁到达木板边界的时刻。
     假设一只蚂蚁在位置 p。如果这只蚂蚁向左移动，则它到达木板边界需要的时间是 p。
     如果这只蚂蚁向右移动，则它到达木板边界需要的时间是 n−p。
     遍历数组 left 和 right，
     根据每只蚂蚁的初始位置和移动方向得到每只蚂蚁到达木板边界需要的时间，
     其中的最大值即为最后一只蚂蚁到达木板边界的时刻，也是最后一只蚂蚁从木板上掉下来的时刻。
    */
    public int getLastMoment(int n, int[] left, int[] right) {
        int lastMoment = 0;
        for (int p : left) {
            lastMoment = Math.max(lastMoment, p);
        }
        for (int p : right) {
            lastMoment = Math.max(lastMoment, n - p);
        }
        return lastMoment;
    }
}